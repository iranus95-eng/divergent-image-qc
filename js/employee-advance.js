...
(function(){
  ...
  function addStyles(){
    ...
    const st=document.createElement('style');st.id='employeeAdvanceStyle';st.textContent=`
      ...
      .adv-table-wrap{overflow:auto}.adv-table{width:100%;border-collapse:collapse;min-width:930px}
      ...
      .adv-actions{display:flex;gap:6px}.adv-approve{background:#16a34a!important;padding:7px 11px!important;font-size:12px!important}.adv-reject{background:#dc2626!important;padding:7px 11px!important;font-size:12px!important}
  ...
  function build(){
    ...
    if(!document.getElementById('employeeAdvanceWorkspace')){
      const w=document.createElement('section');w.className='payroll-workspace';w.id='employeeAdvanceWorkspace';w.innerHTML=`
        ...
        <div class="adv-tabs">
          <button class="adv-tab active" data-status="ALL">ทั้งหมด</button><button class="adv-tab" data-status="PENDING">รออนุมัติ</button><button class="adv-tab" data-status="APPROVED">อนุมัติแล้ว</button><button class="adv-tab" data-status="REJECTED">ไม่อนุมัติ</button>
        </div>
  ...
  function hideOthers(){
    ...
    const nav=document.getElementById('navEmployeeAdvance');if(nav)nav.classList.add('active');
    document.querySelectorAll('[id$="Workspace"]').forEach(el=>{if(el.id!=='employeeAdvanceWorkspace')el.style.display='none';});
    const w=document.getElementById('employeeAdvanceWorkspace');if(w)w.style.display='block';
  }
  ...
  async function open(){
    if(window.DivergentAuth&&window.DivergentAuth.requireAccess&&!window.DivergentAuth.requireAccess('payroll','เมนูพนักงานเบิกเงินล่วงหน้า'))return;
    hideOthers();window.scrollTo(0,0);await reload();
  }

  function render(data){
    document.getElementById('advCount').textContent=String(data.totals?.count||0);
    document.getElementById('advTotal').textContent=money(data.totals?.total_amount||0)+' บาท';
    document.getElementById('advPending').textContent=String(data.totals?.pending||0);
    document.getElementById('advApproved').textContent=String(data.totals?.approved||0);
    const root=document.getElementById('advContent');
    if(!data.sites||!data.sites.length){root.className='adv-empty';root.innerHTML='ยังไม่มีรายการเบิกเงินล่วงหน้า';return;}
    root.className='';root.innerHTML=data.sites.map(g=>`<div class="adv-site"><div class="adv-site-head"><div class="adv-site-title">📍 ไซต์งาน: ${esc(g.site_name)} <span class="adv-note">(${g.employee_count} รายการ)</span></div><div class="adv-site-total">รวม ${money(g.total_amount)} บาท</div></div><div class="adv-table-wrap"><table class="adv-table"><thead><tr><th>#</th><th>ชื่อพนักงาน</th><th>จำนวนเงิน</th><th>วันที่ขอ</th><th>เหตุผล</th><th>LINE</th><th>สถานะ</th><th>จัดการ</th></tr></thead><tbody>${g.items.map((x,i)=>`<tr><td>${i+1}</td><td><b>${esc(x.employee_name)}</b><div class="adv-note">${esc(x.request_code||'')}</div></td><td><b>${money(x.requested_amount)} บาท</b></td><td>${new Date(x.created_at).toLocaleString('th-TH')}</td><td>${esc(x.request_note||'-')}</td><td>${x.line_linked?'เชื่อมแล้ว':'ยังไม่เชื่อม'}</td><td><span class="${statusClass(x.status)}">${statusLabel(x.status)}</span></td><td>${x.status==='PENDING'?`<div class="adv-actions"><button class="adv-approve" onclick="window.EmployeeAdvance.approve(${Number(x.id)},'${esc(x.employee_name).replace(/'/g,'&#39;')}',${Number(x.requested_amount)})">อนุมัติ</button><button class="adv-reject" onclick="window.EmployeeAdvance.reject(${Number(x.id)},'${esc(x.employee_name).replace(/'/g,'&#39;')}')">ไม่อนุมัติ</button></div>`:'-'}</td></tr>`).join('')}</tbody></table></div></div>`).join('');
  }

  async function reload(){
    const err=document.getElementById('advError');if(err){err.style.display='none';err.textContent='';}
    const root=document.getElementById('advContent');if(root){root.className='adv-loading';root.innerHTML='กำลังโหลดข้อมูล...';}
    try{render(await call({action:'list',status:currentStatus}));}catch(e){if(err){err.textContent=e.message||String(e);err.style.display='block';}if(root){root.className='adv-empty';root.innerHTML='โหลดข้อมูลไม่สำเร็จ';}}
  }

  async function approve(id,name,amount){
    if(!confirm(`ยืนยันอนุมัติยอด ${money(amount)} บาท ของ ${name}?\n\nระบบจะบันทึกยอดนี้ลงคอลัมน์เบิกล่วงหน้าในเงินเดือนอัตโนมัติ`))return;
    try{const j=await call({action:'approve',request_id:id});let msg=`อนุมัติเรียบร้อย\nยอดเบิกล่วงหน้าในเงินเดือนปัจจุบัน: ${money(j.approval?.advance_deduction||0)} บาท`;if(j.line?.ok)msg+='\nส่งข้อความ LINE ให้พนักงานแล้ว';else msg+=`\nLINE: ${j.line?.reason||'ยังไม่ได้ส่ง'}`;alert(msg);await reload();}catch(e){alert('อนุมัติไม่สำเร็จ: '+(e.message||e));}
  }
  ...
  window.EmployeeAdvance={open,reload,approve,reject};
  window.openEmployeeAdvanceManagement=open;
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',build);else build();
  window.addEventListener('divergent:permissions',syncVisibility);
})();
