...
function ensureElement(el,name){if(!el)return null;if(el.tagName==='SELECT'){const n=document.createElement(name==='address'?'textarea':'input');n.name=name;n.value=el.value||'';n.className=el.className||'';if(name==='address')n.rows=2;el.replaceWith(n);el=n}el.readOnly=false;el.removeAttribute('aria-readonly');return el}
function ensureInput(root,name){return ensureElement(source(root,name),name)}

function style(){if(document.getElementById('rt-smart-dropdown-style-v5'))return;const s=document.createElement('style');s.id='rt-smart-dropdown-style-v5';s.textContent=`
...
#receiptTaxV1 .rt-add-row{display:flex!important;justify-content:flex-start!important;padding:2px 12px 12px!important}
#receiptTaxV1 #rtAddItem{display:inline-flex!important;align-items:center!important;justify-content:center!important;min-width:120px!important}
...
function pickerForElement(el,name,options,placeholder,onPick){
  el=ensureElement(el,name);if(!el)return null;
  const field=el.closest('.rt-field');if(!field)return null;
  el.classList.add('rt-smart-source');
  let sel=field.querySelector(`.rt-smart-picker[data-for="${name}"]`);
  if(!sel){
    sel=document.createElement('select');sel.className='rt-smart-picker';sel.dataset.for=name;sel.setAttribute('aria-label',placeholder||name);field.insertBefore(sel,el);
    if(!field.querySelector('.rt-smart-help')){const help=document.createElement('div');help.className='rt-smart-help';help.textContent='เลือกจากรายการ หากไม่มีให้เลือก “พิมพ์ข้อความอื่นเอง”';field.appendChild(help)}
    sel.addEventListener('change',()=>{
      if(sel.value==='__manual__'){el.classList.remove('rt-smart-hidden');el.focus();return}
      if(!sel.value)return;
      setValue(el,sel.value,true);
      if(typeof onPick==='function')onPick(sel.value,el,sel);
    });
    if(!el.dataset.rtHistoryBound){el.addEventListener('change',()=>remember(name,el.value));el.dataset.rtHistoryBound='1'}
  }
  syncPicker(sel,options,el.value,placeholder);
  if(sel.value==='__manual__')el.classList.remove('rt-smart-hidden');else el.classList.add('rt-smart-hidden');
  return sel;
}
function picker(root,name,options,placeholder,onPick){return pickerForElement(ensureInput(root,name),name,options,placeholder,onPick)}
function cardPicker(card,name,options,placeholder,onPick){return pickerForElement(card.querySelector(`[name="${name}"]`),name,options,placeholder,onPick)}

function ensureAddButton(root){
  if(root.querySelector('#rtAddItem'))return;
  const title=[...root.querySelectorAll('.rt-section-title')].find(x=>(x.textContent||'').includes('รายการและยอดเงิน'));
  const section=title?.closest('.rt-section');if(!section)return;
  const row=document.createElement('div');row.className='rt-add-row';row.innerHTML='<button type="button" class="rt-btn secondary" id="rtAddItem">+ เพิ่มรายการ</button>';
...
function refresh(root,data){
  ...
  cards.forEach(card=>{
    const item=card.querySelector('[name="item"]');
    const detail=card.querySelector('[name="detail"]');
    const period=card.querySelector('[name="period"]');
    const site=card.querySelector('[name="site"]');
    const itemMasterValues=rows.map(x=>x.item_name||x.item||x.service_name||'');
    const detailMasterValues=rows.map(x=>x.detail||x.description||'');
    if(item)cardPicker(card,'item',opts(['ค่าจ้างจัดพิมพ์ (P00001)',...itemMasterValues,item.value,...hist('item')]),'-- เลือกรายการ --');
    if(detail)cardPicker(card,'detail',opts(['ค่าจ้างจัดพิมพ์ใบแจ้งเตือนและส่งใบแจ้งเตือนค่าไฟฟ้า',...detailMasterValues,detail.value,...hist('detail')]),'-- เลือกรายละเอียด --');
    if(period)cardPicker(card,'period',opts([...rows.map(x=>x.billing_text),period.value,...hist('period')]),'-- เลือกรอบบิล / เดือน --');
    if(site)cardPicker(card,'site',opts(['เขตชุมชน','นอกเขตชุมชน',...rows.map(x=>x.area_name),site.value,...hist('site'),...hist('area')]),'-- เลือกพื้นที่ / เขตงาน --',v=>{
      const hit=rows.find(x=>norm(x.area_name)===norm(v));
      if(hit&&period&&hit.billing_text){setValue(period,hit.billing_text,true);const ps=period.closest('.rt-field')?.querySelector('.rt-smart-picker[data-for="period"]');if(ps)syncPicker(ps,opts([...rows.map(x=>x.billing_text),...hist('period')]),period.value,'-- เลือกรอบบิล / เดือน --')}
    });
    card.dataset.rtSmartCardV5='1';
  });
...
function customerPicker(root,data){
  const el=ensureInput(root,'customer');if(!el)return;const field=el.closest('.rt-field');if(!field)return;
  el.classList.add('rt-smart-source');let sel=field.querySelector('.rt-smart-picker[data-for="customer"]');
  if(!sel){sel=document.createElement('select');sel.className='rt-smart-picker';sel.dataset.for='customer';field.insertBefore(sel,el);const help=document.createElement('div');help.className='rt-smart-help';help.textContent='เลือกการไฟฟ้าแล้ว ระบบเติมที่อยู่ เลขภาษี และสาขาให้อัตโนมัติ';field.appendChild(help);sel.addEventListener('change',()=>{if(sel.value==='__manual__'){el.classList.remove('rt-smart-hidden');el.focus();return}if(!sel.value)return;const c=data.customers.find(x=>String(x.id)===sel.value);if(c)applyCustomer(root,data,c)});el.addEventListener('change',()=>remember('customer',el.value))}
  const current=customerMatch(data.customers,el.value,ensureInput(root,'branch')?.value);
  sel.innerHTML='<option value="">-- เลือกการไฟฟ้า / หน่วยงาน --</option>'+data.customers.map(c=>`<option value="${esc(c.id)}">${esc((c.branch_code||'')+' · '+(c.customer_name||''))}</option>`).join('')+'<option value="__manual__">✎ พิมพ์หน่วยงานอื่นเอง</option>';
  if(current){root.dataset.rtCustomerId=String(current.id||'');sel.value=String(current.id);el.classList.add('rt-smart-hidden')}else if(el.value){sel.value='__manual__';el.classList.remove('rt-smart-hidden')}else{sel.value='';el.classList.add('rt-smart-hidden')}
}

async function enhance(root){
  if(!root||root.dataset.rtSmartV5==='loading')return;
  root.dataset.rtSmartV5='loading';style();
  try{const data=await loadMaster();if(!document.body.contains(root))return;customerPicker(root,data);const cur=customerMatch(data.customers,ensureInput(root,'customer')?.value,ensureInput(root,'branch')?.value);if(cur)root.dataset.rtCustomerId=String(cur.id||'');refresh(root,data);root.dataset.rtSmartV5='1'}catch(e){root.dataset.rtSmartV5='error';console.error('receipt smart dropdowns v5',e)}finally{if(root.dataset.rtSmartV5==='loading')root.dataset.rtSmartV5='0'}
}
async function refreshNewCards(root){
  if(!root||root.dataset.rtSmartV5!=='1')return;
  if(![...root.querySelectorAll('.rt-item-editor')].some(card=>card.dataset.rtSmartCardV5!=='1'))return;
  try{const data=await loadMaster();refresh(root,data)}catch(_){ }
}
function scan(){const root=document.getElementById('receiptTaxV1');if(!root)return;if(root.dataset.rtSmartV5!=='1')enhance(root);else refreshNewCards(root)}
