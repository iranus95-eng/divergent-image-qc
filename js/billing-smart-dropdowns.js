...
async function enhance(root){if(!root||root.dataset.biSmartV4==='loading'||root.dataset.biSmartV4==='1')return;root.dataset.biSmartV4='loading';style();try{const data=await loadMaster();if(!document.body.contains(root))return;customerPicker(root,data);const cur=customerByValue(data,source(root,'customer')?.value,source(root,'branch')?.value);if(cur)root.dataset.biCustomerId=String(cur.id);refresh(root,data);root.dataset.biSmartV4='1'}catch(e){root.dataset.biSmartV4='error';console.error('billing smart dropdowns v4',e);const box=root.querySelector('.bi-body');if(box&&!box.querySelector('.bi-smart-help-error')){const d=document.createElement('div');d.className='bi-smart-help bi-smart-help-error';d.style.color='#a42323';d.textContent='โหลดรายการ Dropdown ไม่สำเร็จ กรุณาเปิดเมนูใหม่';box.prepend(d)}}}
function scan(){const root=document.getElementById('billingInvoiceV1');if(root&&root.dataset.biSmartV4!=='1')enhance(root)}
