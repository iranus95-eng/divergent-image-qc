alter table public.legacy_staff_payroll_drafts rename to staff_payroll_monthly_drafts;
alter sequence public.legacy_staff_payroll_drafts_id_seq rename to staff_payroll_monthly_drafts_id_seq;
alter function public.ensure_legacy_staff_payroll_month(date) rename to ensure_staff_payroll_month;
alter function public.legacy_staff_payroll_rows(date) rename to staff_payroll_report_rows;

create or replace function public.ensure_staff_payroll_month(p_month date)
...
begin
  ...
  if exists(select 1 from public.staff_payroll_monthly_drafts where payroll_month=p_month) then
    return 0;
  end if;
  select max(payroll_month) into v_source from (
    ...
    union all
    select payroll_month from public.staff_payroll_monthly_drafts where is_active and payroll_month<p_month
  ) m;
  ...
  with roster as (
    ...
    from public.staff_payroll_monthly_drafts d where payroll_month=v_source and is_active
      and not exists(select 1 from public.staff_payroll_rows where payroll_month=v_source)
  )
  insert into public.staff_payroll_monthly_drafts
  ...
  select p_month,r.source_row,r.national_id,r.employee_name,r.bank_name,r.bank_account,r.start_date_text,
    ...
    'รอกรอกเงินเดือนหลังเสร็จงาน','V2_MONTHLY_DRAFT'
  from roster r
  left join lateral (
...
create or replace function public.staff_payroll_report_rows(p_month date)
returns jsonb language sql stable security invoker set search_path = '' as $$
  with source as (
    select to_jsonb(s) as row_data,s.source_row,s.national_id,s.employee_name,
           ...
           (select d.payroll_profile_id from public.staff_payroll_monthly_drafts d
    ...
    from public.staff_payroll_monthly_drafts d
...
revoke all on function public.ensure_staff_payroll_month(date) from public, anon, authenticated;
grant execute on function public.ensure_staff_payroll_month(date) to service_role;
revoke all on function public.staff_payroll_report_rows(date) from public, anon, authenticated;
grant execute on function public.staff_payroll_report_rows(date) to service_role;
