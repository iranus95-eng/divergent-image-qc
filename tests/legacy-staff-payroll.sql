...
begin
  ...
  rows:=public.legacy_staff_payroll_rows('2026-09-01');
  ...
  select d.source_row into target_row from public.legacy_staff_payroll_drafts d
  ...
  select d.payroll_profile_id,d.source_row into profile,target_row from public.legacy_staff_payroll_drafts d
    join public.employee_advance_requests a on a.payroll_profile_id=d.payroll_profile_id
  ...
  update public.legacy_staff_payroll_drafts set net_paid=12345.67,advance_deduction=10,
  ...
  if rows<>public.legacy_staff_payroll_rows('2026-09-01') then raise exception 'double deduction'; end if;
  ...
  n:=public.ensure_legacy_staff_payroll_month('2026-10-01');
  if n<>4 then raise exception 'new month roster missing'; end if;
  if public.ensure_legacy_staff_payroll_month('2026-10-01')<>0 then raise exception 'duplicate month'; end if;
  rows:=public.legacy_staff_payroll_rows('2026-10-01');
  ...
  insert into public.staff_payroll_rows
    ...
    from public.legacy_staff_payroll_drafts where payroll_month='2026-09-01';
  rows:=public.legacy_staff_payroll_rows('2026-09-01');
  ...
  insert into public.staff_payroll_rows
    ...
    from public.legacy_staff_payroll_drafts where payroll_month='2026-09-01';
  if public.ensure_legacy_staff_payroll_month('2026-11-01')<>4 then
    raise exception 'preexisting payroll identity mapping missing'; end if;
  if has_function_privilege('anon','public.legacy_staff_payroll_rows(date)','execute')
     or has_table_privilege('authenticated','public.legacy_staff_payroll_drafts','select') then
    raise exception 'unauthorized access'; end if;
end $$;
set local role service_role;
select public.legacy_staff_payroll_rows('2026-09-01') is not null as service_role_can_read;
rollback;
